...
$(document).ready(function() {
  ...
  const selectedDate = new Date($('#ctrl-tanggal').val());
  // get the day of the week (0-6)
  const dayOfWeek = selectedDate.getDay();
  // define an array of day names
  const days = ["Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];
  // display the name of the day based on the day of the week
  $('#ctrl-hari').val(days[dayOfWeek]);
  // add event listener to the input date element
  $('#ctrl-tanggal').on('input', function() {
    // get the selected date value
...
$('#ctrl-jam_pulang').on('input', function() {
  var hari = $('#ctrl-hari').val();
  var jam_datang = $('#ctrl-jam_datang').val();
  var jam_pulang = $('#ctrl-jam_pulang').val();
  var jam_1 = (hari == 'Sabtu' || hari == 'Minggu') ? 0 : 1;
  var jam_2 = Math.min(jam_pulang - jam_datang - jam_1, 8);
  if (jam_2 > 7 && hari != 'Sabtu' && hari != 'Minggu') {
    jam_2 = 7;
  }
  var jam_3 = (jam_pulang - jam_datang > 8) ? 1 : 0;
  var jam_4 = 0;
  if (jam_pulang - jam_datang > 9) {
    jam_4 = jam_pulang - jam_datang - 9;
  }
  $('#ctrl-jam_1').val(jam_1);
  $('#ctrl-jam_2').val(jam_2);
  $('#ctrl-jam_3').val(jam_3);
  $('#ctrl-jam_4').val(jam_4);
});
$(document).ready(function() {
  // Event listener untuk input pada kolom id_material
  $('#ctrl-id_material').on('input', function() {
    // Ambil nilai id_material dari kolom yang sedang diinput
    var id_material = $(this).val();
    // Jika id_material tidak kosong
    if (id_material !== '') {
      // Lakukan request AJAX untuk mengambil data harga_satuan dan vendor
      $.ajax({
        url: '/aplikasidimas/script/get_data.php',
        type: 'GET',
        data: {id_material: id_material},
        success: function(response) {
          // Set nilai harga_satuan dan vendor dari response ke dalam masing-masing kolom
          var data = JSON.parse(response);
          $('#ctrl-harga_satuan').val(data.harga_satuan);
          $('#ctrl-vendor').val(data.vendor);
          // Hapus opsi vendor sebelumnya
          $('#ctrl-vendor option').remove();
          // Tambahkan opsi vendor baru ke dalam select box
          $.each(data.vendors, function(index, value) {
            $('#ctrl-vendor').append($('<option>').text(value).attr('value', value));
          });
        },
      ...
      // Kosongkan nilai harga_satuan dan opsi vendor jika id_material kosong
      $('#ctrl-harga_satuan').val('');
      $('#ctrl-vendor').val('');
    }
  });
});
$(document).ready(function() {
  $('#ctrl-kategori').on('change', function() {
    var kategoriVal = $(this).val();
    if (kategoriVal === 'MASUK') {
      $("label[for='tanggal_masuk']").show();
      $('#ctrl-tanggal_masuk').show();
      $("label[for='tanggal_inpro']").hide();
      $('#ctrl-tanggal_inpro').hide();
    } else if (kategoriVal === 'INPROGRES') {
      $("label[for='tanggal_inpro']").show();
      $('#ctrl-tanggal_inpro').show();
      $("label[for='tanggal_masuk']").hide();
      $('#ctrl-tanggal_masuk').hide();
    } else {
      $("label[for='tanggal_masuk']").hide();
      $('#ctrl-tanggal_masuk').hide();
      $("label[for='tanggal_inpro']").hide();
...
$(document).ready(function() {
  // Event listener untuk input pada kolom harga_satuan
  $('#ctrl-harga_satuan').on('input', function() {
    // Ambil nilai jumlah dari kolom jumlah
    var jumlah = $('#ctrl-jumlah').val();
    // Ambil nilai harga_satuan dari kolom harga_satuan
    var harga_satuan = $(this).val();
    // Jika jumlah dan harga_satuan tidak kosong
    if (jumlah !== '' && harga_satuan !== '') {
      // Hitung harga dari hasil perkalian harga_satuan dengan jumlah
      var harga = parseFloat(harga_satuan) * parseInt(jumlah);
      // Set nilai harga ke dalam kolom harga dengan format Rupiah
      $('#ctrl-harga').val('Rp ' + harga.toLocaleString());
    }
  });
});
$(document).ready(function() {
  // Event listener untuk perubahan pada kolom jumlah
  $('#ctrl-jumlah').change(function() {
    updateHarga();
  });
  // Event listener untuk perubahan pada kolom harga satuan
  $('#ctrl-harga_satuan').change(function() {
    updateHarga();
  });
  // Fungsi untuk mengupdate nilai di kolom harga
  function updateHarga() {
    // Ambil nilai jumlah dari kolom jumlah
    var jumlah = $('#ctrl-jumlah').val();
    // Ambil nilai harga_satuan dari kolom harga_satuan
    var harga_satuan = $('#ctrl-harga_satuan').val();
    // Jika jumlah dan harga_satuan tidak kosong
    if (jumlah !== '' && harga_satuan !== '') {
      // Hitung harga dari hasil perkalian harga_satuan dengan jumlah
      var harga = parseFloat(harga_satuan) * parseInt(jumlah);
